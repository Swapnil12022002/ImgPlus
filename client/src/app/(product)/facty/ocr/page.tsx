...
import { ocrResponse } from "@/types/responseTypes";
import { createOCR } from "@/api/product";
...
const OCR: React.FC = () => {
  ...
  const [responseArray, setResponseArray] = useState<ocrResponse["text"] | []>(
    []
  );
  ...
  useEffect(() => {
    if (response) {
      setResponseArray(response.text);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const serverResponse = await createOCR(formData);
      ...
      <div className="max-w-lg mx-auto mt-12">
        {responseLoading ? (
          ...
          <div className="dark:text-white text-black flex flex-col justify-center items-center gap-3">
            ...
            {responseArray.map((text, index) => (
              <p key={index}>{text}</p>
            ))}
          </div>
        ) : response && responseArray && responseArray?.length === 0 ? (
